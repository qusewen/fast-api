...
from app.Models.auth.auth_models import UserCreate, RegisterResponse, UserResponse, ResetPasswordResponse, ResetResponse
...
async def register(user: UserCreate, db: asyncpg.Connection = Depends(get_db_connection)):
    ...
    await db.execute('''
         ...
         VALUES($1, $2, $3, $4)
    ''', user.name, user.email, hashed, user.age)
...
async def reset_password(pas: ResetPasswordResponse, db: asyncpg.Connection = Depends(get_db_connection)):
    ...
    if not user:
        ...
        if verify_password(pas.prev_password, dict_user['password']):
            if verify_password(pas.new_password, dict_user['password']):
                ...
                await db.execute('UPDATE users SET password = $1 WHERE email = $2', hashed, pas.email)
